...
def cache_result(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Only save if result is a DataFrame
...
def get_easiest_subjects():
    ...
    for subj, (s, t) in success.items():
        ...
        data.append({
            "Subject": subj,
            "Description": subj_info["Description"] if subj_info else "",
            "High Grades (>=90)": f"{s/t:.0%}",
            "Students": t
        })
    return pd.DataFrame(data).sort_values("High Grades (>=90)", ascending=False)

...
class data_helper(object):
    ...
    def __init__(self, arg):
        super(data_helper, self).__init__()
        self.arg = arg
        self.db = arg["db"]

    def find_best_match(self,query=None, course=None, collection=None, limit=200):

        if collection is None:
            raise ValueError("⚠️ You must pass a MongoDB collection (e.g., self.db.students).")
    ...
    def get_enrolled_students(self,course=None, semester_id=None):
        """
    ...
    def approve_enrollee(self,student_id: str, semester_id: int, subject_codes: List[str], approved_by_user: dict):
        """
        Approves a pending enrollment, moves status to 'Enrolled',
        and creates a single grade entry (with arrays) for the semester.
        """
        enrollment = self.db.enrollments.find_one({"studentId": int(student_id), "semesterId": int(semester_id)})
        ...
        # 🔎 Get teachers for each subject from self.db.subjects
        teachers_list = []
        for subj_code in subject_codes_list:
            subject_doc = self.db.subjects.find_one({"_id": subj_code}, {"Teacher": 1})
            teachers_list.append(subject_doc.get("Teacher", "") if subject_doc else "")

        grade_doc = {
            "_id": (self.db.grades.find_one(sort=[("_id", -1)])["_id"] + 1) if self.db.grades.count_documents({}) else 1,
        ...
        self.db.grades.insert_one(grade_doc)

        print(f"✅ Enrollment approved and grade record created for {len(subjects)} subjects.")
        return enrollment["_id"]

    def add_pending_enrollee(self,student_id, semester_id, subject_codes=None, registered_by_user=None):
        """
        ...
        try:
            ...
            semester = self.db.semesters.find_one({"_id": semester_id})
            if not semester:
            ...
            result = self.db.enrollments.insert_one(enrollment_doc)
    ...
    def discard_pending_enrollee(self,student_id, semester_id, discarded_by_user=None, reason=None):
        """
    ...
    def get_pending_enrollees(self,semester_id=None, course=None):
        """
        ...
        try:
            ...
            pending_docs = list(self.db.enrollments.find(query))
            if not pending_docs:
            ...
            for enrollment in pending_docs:
                ...
                student_doc = self.db.students.find_one({"_id": enrollment["studentId"]})
    ...
    def get_discarded_enrollees(self,semester_id=None):
        """
        ...
        try:
            ...
            discarded_docs = list(self.db.enrollments.find(query))
    ...
    def update_enrollment(self,student_id, semester_id, add_subjects=None, drop_subjects=None, updated_by_user=None):
        """
        ...
        try:
            ...
            grade_doc = self.db.grades.find_one({"StudentID": student_id, "SemesterID": semester_id})
            if grade_doc:
    ...
    def is_already_enrolled(self,student_id, semester_id):
        """
        Check if a student is already enrolled (to prevent duplicates).
        """
        try:
            exists = self.db.enrollments.find_one({
                "studentId": student_id,
                "semesterId": semester_id,
    ...
    if __name__ == "__main__":
        ...
        if enrollment:
            ...
            self.db.enrollments.update_one({"_id": enrollment["_id"]}, {"$set": {"subjects": subjects}})
